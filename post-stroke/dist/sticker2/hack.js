...
function tweet(resp, text) {
    ...
    if(data && data.appId) {
        _uploading = false;
        var hash = ["#enchantMOON", "#enchantmoonstrokes"];
        MOON.openUrl("https://twitter.com/intent/tweet?source=webclient&text=" + encodeURIComponent((text ? text + " " : "") + data.appId + " " + hash.join(" ")));
        MOON.finish();
    } else {
        _uploading = false;
        fail();
